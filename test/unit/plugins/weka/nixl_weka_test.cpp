...
int main(int argc, char *argv[])
{
    ...
    unsigned int                ngpus = DEFAULT_NUM_GPUS;
    ...
    while ((opt = getopt_long(argc, argv, "dvn:s:rwp:b:m:t:G:Dh", long_options, NULL)) != -1) {
        switch (opt) {
            case 'd':
                use_dram = true;
                break;
    ...
    nixlBackendH                *weka;
    nixl_reg_dlist_t            vram_for_weka(VRAM_SEG);
    nixl_reg_dlist_t            dram_for_weka(DRAM_SEG);
    nixl_reg_dlist_t            file_for_weka(FILE_SEG);
    std::string                 name;

    std::cout << "\n============================================================" << std::endl;
    std::cout << "                 NIXL STORAGE TEST STARTING (WEKA PLUGIN)                     " << std::endl;
    std::cout << "============================================================" << std::endl;
    ...
    nixlAgent agent("WEKATester", cfg);

    // Set Weka backend parameters
    params["max_request_size"] = std::to_string(max_request_size);

    // Create backends
    ret = agent.createBackend("WEKA", params, weka);
    if (ret != NIXL_SUCCESS || weka == NULL) {
        std::cerr << "Error creating WEKA backend: "
    ...
    for (i = 0; i < num_transfers; i++) {
        ...
        if (use_vram) {
            ...
            vram_for_weka.addDesc(vram_buf[i]);
        }
        ...
        if (use_dram) {
            ...
            dram_for_weka.addDesc(dram_buf[i]);
        }

        ftrans[i].addr  = 0;
        ftrans[i].len   = transfer_size;
        ftrans[i].devId = fd[i];
        file_for_weka.addDesc(ftrans[i]);

        printProgress(float(i + 1) / num_transfers);
    }

    std::cout << "\n=== Registering memory ===" << std::endl;
    ret = agent.registerMem(file_for_weka);
    if (ret != NIXL_SUCCESS) {
    ...
    if (use_vram) {
        ret = agent.registerMem(vram_for_weka);
        if (ret != NIXL_SUCCESS) {
    ...
    if (use_dram) {
        ret = agent.registerMem(dram_for_weka);
        if (ret != NIXL_SUCCESS) {
    ...
    nixl_xfer_dlist_t file_for_weka_list = file_for_weka.trim();
    nixl_xfer_dlist_t src_list = use_dram ? dram_for_weka.trim() : vram_for_weka.trim();
...
cleanup:
    ...
    // Cleanup resources
    agent.deregisterMem(file_for_weka);
    if (use_vram) {
        agent.deregisterMem(vram_for_weka);
        for (i = 0; i < num_transfers; i++) {
    ...
    if (use_dram) {
        agent.deregisterMem(dram_for_weka);
        for (i = 0; i < num_transfers; i++) {
