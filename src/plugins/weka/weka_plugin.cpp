...
static const char* PLUGIN_NAME = "WEKA";
static const char* PLUGIN_VERSION = "0.1.0";

static nixlBackendEngine* create_weka_engine(const nixlBackendInitParams* init_params) {
    return new nixlWekaEngine(init_params);
}

static void destroy_weka_engine(nixlBackendEngine* engine) {
    delete engine;
}
...
nixlBackendPlugin* createStaticWekaPlugin() {
